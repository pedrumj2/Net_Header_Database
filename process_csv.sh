...
if [ $1 == "-h" ]; then
  ...
  COPY_MAC_QUERY_SRC="Insert ignore into mydb.macaddr (M1, M2, M3, M4, M5, M6)
                      ...
                      CONVERT(SM6, unsigned int) FROM mydb.CSV);"
  COPY_MAC_QUERY_DST="Insert ignore into mydb.macaddr (M1, M2, M3, M4, M5, M6)
                      ...
                      CONVERT(DM6, unsigned int) FROM mydb.CSV);"
  
  
  COPY_IP_QUERY_SRC="Insert ignore into mydb.ipaddr (IP1, IP2, IP3, IP4) 
                      (SELECT distinct  CONVERT(SIP1, unsigned int),
                      CONVERT(SIP2, unsigned int), CONVERT(SIP3, unsigned int), CONVERT(SIP4, unsigned int) from mydb.CSV);"
  COPY_IP_QUERY_DST="Insert ignore into mydb.ipaddr (IP1, IP2, IP3, IP4) 
                      (SELECT distinct  CONVERT(DIP1, unsigned int),
                      CONVERT(DIP2, unsigned int), CONVERT(DIP3, unsigned int), CONVERT(DIP4, unsigned int) from mydb.CSV);"
                      
                  
                      
  INSERT_PACKETS_QUERY="Insert into mydb.packets(time, macSrc, macDst, vlanTag, ethType, ipSrc, 
 	                      ...
 	                      SELECT  time + time_u/1000000 as time, 
                                ...
                                1  
                      FROM mydb.CSV
                      inner join mydb.macaddr as MAC1
                      	on 	CSV.SM1 = MAC1.M1 and  
                            ...
                            CSV.SM5 = MAC1.M5 and   
                            CSV.SM6 = MAC1.M6  
                      inner join mydb.macaddr as MAC2
